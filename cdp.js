...
// leak-guard (v2.2.1, 2026-09-06): reap the detached chromium group on
// ANY exit path - signal or normal. Without this, a TERM'd/KILL'd
// wrapper orphaned the headless tree (observed ~42-proc leak).
const reap=()=>{try{process.kill(-chrome.pid,'SIGKILL')}catch(e){}};
for(const s of ['SIGTERM','SIGINT','SIGHUP']) process.on(s,()=>{reap();process.exit(1)});
